function display_spectra_mosaic(mrs_spectra,mrs_spectra_recon,origdata_max)
...
%         mrs_spectra_recon = mrs_spectra_recon/max(abs(mrs_spectra_recon(:)));
    ...
    for k1=1:num_col2
        s =[];
        for k2 =1:num_col1
             s_new = squeeze(real(mrs_spectra(k1,k2,:)));
             s = [s; s_new]; 
        end
         zz(k1,:) = s;
    end
...
for k1=1:num_col2
        s =[];
        for k2 =1:num_col1
             s_new = squeeze(real(mrs_spectra_recon(k1,k2,:)));
             s = [s; s_new]; 
        end
         zz2(k1,:) = s;
end
...
%         end
        ...
        plot (squeeze(zz(i,:)),'r');axis tight;%axis([0 (Nsamples*num_col1) 0 1]);
        %hold on;
%                 plot (squeeze(zz(i,:)),'r');axis([0 (Nsamples*num_col1) 0 1]);hold on;

        %plot (squeeze(zz2(i,:)),'r');axis tight;%axis([0 (Nsamples*num_col1) 0 1]);
        %hold on;
        set(gca,'xtick',[],'ytick',[]);
